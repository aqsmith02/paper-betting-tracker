...
class EVAnalyzer:
    ...
    def _create_ev_bins(self):
        ...
        # Define EV bins
        ev_bins = [0.05, 0.075, 0.10, 0.125, 0.15, 0.175, 0.20, 0.225, 0.25, 0.275, 0.30, 0.35, 0.40, 0.45, 0.50, 1.0]
        ev_labels = [
            '5-7.5%', '7.5-10%', '10-12.5%', '12.5-15%', '15-17.5%', '17.5-20%',
            '20-22.5%', '22.5-25%', '25-27.5%', '27.5-30%', '30-35%', '35-40%',
    ...
    def analyze_ev_vs_actual(self) -> pd.DataFrame:
        ...
        analysis = self.df.groupby('EV_Bin', observed=True).agg({
    ...
    def plot_ev_analysis(self, save_path: Optional[str] = None):
        ...
        if not ev_analysis.empty:
            ...
            ax1.bar([i - width/2 for i in x], ev_analysis['Avg_EV'], 
                   width, label='Expected (EV)', alpha=0.8, color='steelblue')
            ax1.bar([i + width/2 for i in x], ev_analysis['Actual_ROI'], 
                   width, label='Actual (ROI)', alpha=0.8, color='coral')
        ...
        p = np.poly1d(z)
        ax2.plot(predicted.sort_values(), p(predicted.sort_values()), 
                "g-", label=f'Trend: y={z[0]:.2f}x+{z[1]:.2f}', linewidth=2)
        ...
        ax3 = axes[1, 0]
        if not ev_analysis.empty:
            ax3.bar(range(len(ev_analysis)), ev_analysis['Win_Rate'], 
            ...
            ax3.axhline(y=50, color='red', linestyle='--', 
                       label='50% (Breakeven)', linewidth=1)
            ax3.legend()
        ...
        if not ev_analysis.empty:
            ...
            ax4.bar(range(len(ev_analysis)), ev_analysis['Total_Bets'], 
...
def analyze_ev_performance(csv_path: str, starting_bankroll: float = 10000.0,
