...
class NullHypothesisSimulator:
    ...
    def run_simulation(self) -> Dict:
        ...
        results = {
            ...
            "percentile_rank": stats.percentileofscore(
                simulated_profits, actual_profit
            ),
            "z_score": (actual_profit - simulated_profits.mean())
            / simulated_profits.std(),
    ...
    def print_results(self, results: Dict):
        ...
        print(
            f"   Range: [${results['min_simulated_profit']:,.2f}, ${results['max_simulated_profit']:,.2f}]"
        )
        ...
        if results["actual_profit"] > results["ci_upper"]:
            print(
                f"   Your profit (${results['actual_profit']:,.2f}) is ABOVE the 95% confidence interval"
            )
            print(f"   under the null hypothesis of {results['null_ev']*100}% EV.")
        elif results["actual_profit"] > results["mean_simulated_profit"]:
            print(
                f"   Your profit (${results['actual_profit']:,.2f}) is above the expected value"
            )
            print(
                f"   under the null hypothesis, but within the 95% confidence interval."
            )
        else:
            print(
                f"   Your profit (${results['actual_profit']:,.2f}) is below or at the expected value"
            )
    ...
    def plot_simulation_results(self, results: Dict, save_path: Optional[str] = None):
        ...
        ax1 = axes[0, 0]
        ax1.hist(
            simulated_profits, bins=50, alpha=0.7, color="steelblue", edgecolor="black"
        )
        ax1.axvline(
            actual_profit,
            color="red",
        ...
        ax1.set_title(
            f'Distribution of Profits Under Null (EV = {results["null_ev"]*100}%)'
        )
        ax1.legend()
        ax1.grid(alpha=0.3)
        ...
        normal_dist = stats.norm.pdf(
            x, results["mean_simulated_profit"], results["std_simulated_profit"]
        )
