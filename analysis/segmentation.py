...
class SegmentationAnalyzer:
    ...
    def _segment_analysis(self, column: str) -> pd.DataFrame:
        ...
        analysis = (
            self.df.groupby(column, observed=True)
            .agg(
                {
        ...
        analysis["Sharpe_Ratio"] = (analysis["Avg_ROI"] / analysis["ROI_StdDev"]).round(
            3
        )
    ...
    def plot_segmentation_overview(self, save_path: Optional[str] = None):
        ...
        if not bookmaker_data.empty:
            bookmaker_data.head(10).plot(
                kind="barh", y="Avg_ROI", ax=ax1, color="steelblue", alpha=0.8
            )
        ...
        if not sport_data.empty:
            sport_data.plot(kind="barh", y="Avg_ROI", ax=ax2, color="coral", alpha=0.8)
        ...
        if not sport_data.empty:
            sport_data.plot(
                kind="barh", y="Total_Bets", ax=ax5, color="gold", alpha=0.8
            )
