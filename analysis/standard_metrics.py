...
class StandardMetricsCalculator:
    ...
    def _prepare_data(self):
        ...
        self.df["Stake_Pct"] = self.df["Stake_Pct"].clip(
            lower=0, upper=0.025
        )  # Cap at 2.5%
        ...
        self.df["Running_Bankroll"] = (
            self.starting_bankroll + self.df["Cumulative_Profit"]
        )

        # Calculate ROI for each bet
        self.df["ROI"] = (
            self.df["Profit_Loss"] / (self.df["Stake_Pct"] * self.starting_bankroll)
        ) * 100
    ...
    def calculate_all_metrics(self) -> Dict:
        ...
        avg_loss = (
            self.df[~self.df["Bet_Won"]]["Profit_Loss"].mean() if losses > 0 else 0
        )
    ...
    def print_summary(self):
        ...
        print(f"\n RISK METRICS")
        print(
            f"   Max Drawdown: ${metrics['max_drawdown']:,.2f} ({metrics['max_drawdown_pct']}%)"
        )
