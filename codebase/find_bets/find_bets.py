...
def run_betting_strategy(
    strategy: BettingStrategy, vigfree_data: pd.DataFrame, file_manager: BetFileManager
) -> None:
    """
    ...
    try:
        ...
        filtered_summary = file_manager.save_best_bets_only(
            summary, strategy.summary_file
        )

        # Save full data using the same filtered summary
        file_manager.save_full_betting_data(
            analysis_result, filtered_summary, strategy.full_file
        )
