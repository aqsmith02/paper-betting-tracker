...
def _calculate_expected_value(
    probabilities: pd.Series, best_odds: pd.Series
) -> pd.Series:
    """
    ...
    ev[mask] = (probabilities[mask] * (best_odds[mask])) - 1

    return ev.round(2)


def _calculate_modified_zscore(
    values: pd.Series, reference_values: pd.DataFrame
) -> pd.Series:
    """
...
def _filter_by_threshold(
    values: pd.Series, min_threshold: float, max_threshold: float
) -> pd.Series:
    """
...
def find_modified_zscore_bets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    modified_z = _calculate_modified_zscore(df["Best Odds"], df[bookmaker_columns])

    # Apply threshold filters
    modified_z_filtered = _filter_by_threshold(
        modified_z, Z_SCORE_THRESHOLD, MAX_Z_SCORE
    )
