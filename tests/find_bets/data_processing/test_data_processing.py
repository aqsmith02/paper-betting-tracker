...
def sample_odds_df():
    ...
    return pd.DataFrame({
        ...
        'Sport Key': ['basketball_nba', 'basketball_nba', 'basketball_nba', 'basketball_nba'],
        'Sport Title': ['NBA', 'NBA', 'NBA', 'NBA'],
        'Start Time': ['2026-01-25T19:00:00Z'] * 4,
        'Match': ['Warriors @ Lakers', 'Warriors @ Lakers', 'Celtics @ Heat', 'Celtics @ Heat'],
...
class TestFindBookmakerColumns:
    ...
    def test_with_additional_exclusions(self, sample_odds_df):
        ...
        bookmakers = find_bookmaker_columns(sample_odds_df, exclude_columns=['DraftKings', 'FanDuel'])
...
class TestAddMetadata:
    ...
    def test_handles_missing_bookmakers(self):
        ...
        assert all(pd.isna(result['Best Bookmaker']) | (result['Best Bookmaker'] == None))
...
class TestProcessTargetOddsData:
    ...
    def comprehensive_test_df(self):
        ...
        """
        return pd.DataFrame({
            'ID': ['g1', 'g1', 'g2', 'g2', 'g3', 'g4', 'g4', 'g5', 'g5', 'g6', 'g6'],
            'Sport Key': ['basketball_nba'] * 11,
            'Sport Title': ['NBA'] * 11,
            'Start Time': ['2026-01-25T19:00:00Z'] * 11,
            'Match': ['A @ B', 'A @ B', 'C @ D', 'C @ D', 'E @ F', 'G @ H', 'G @ H', 'I @ J', 'I @ J', 'K @ L', 'K @ L'],
            'Team': ['A', 'B', 'C', 'D', 'E', 'G', 'H', 'I', 'J', 'K', 'L'],
            
            # Valid bookmakers (in ALL_BMS)
            'DraftKings': [2.10, 1.85, 1.0, 2.00, 2.50, 2.00, 1.90, 150.0, 1.20, 2.20, 1.75],
            'FanDuel':    [2.05, 1.90, 2.10, 1.95, 2.45, 2.00, 1.90, 145.0, 1.25, 2.15, 1.80],
            'BetMGM':     [2.15, 1.80, 2.05, 1.98, 2.40, 2.00, 1.95, 140.0, 1.22, 2.25, 1.70],
            'Caesars':    [2.08, 1.87, 2.08, 1.96, 2.40, 2.00, 1.88, 140.0, 1.24, 2.18, 1.77],
            'Pinnacle':   [2.20, 2.0, 2.20, 2.05, 2.65, np.nan, 2.00, 165.0, 1.30, 2.25, 1.85],

            
            # Unwanted bookmakers (not in ALL_BMS) - should be removed
            'UnknownBook1': [2.00, 1.95, 2.00, 2.00, 2.30, 2.10, 1.85, 130.0, 1.30, 2.10, 1.85],
            'UnknownBook2': [2.12, 1.88, 2.12, 1.92, 2.35, 2.05, 1.92, 135.0, 1.28, 2.12, 1.82]
        })
    ...
    def test_complete_pipeline_integration(self, comprehensive_test_df):
        ...
        team_a_row = result[(result['Match'] == 'A @ B') & (result['Team'] == 'A')].iloc[0]
        team_b_row = result[(result['Match'] == 'A @ B') & (result['Team'] == 'B')].iloc[0]
        team_k_row = result[(result['Match'] == 'K @ L') & (result['Team'] == 'K')].iloc[0]
        team_l_row = result[(result['Match'] == 'K @ L') & (result['Team'] == 'L')].iloc[0]
