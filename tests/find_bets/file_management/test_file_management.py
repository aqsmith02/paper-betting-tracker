        ...
        df = pd.DataFrame(
            {
                ...
                "Start Time": pd.to_datetime(
                    ["2024-01-15 19:00", "2024-01-15 19:00", "2024-01-16 19:00"]
                ),
...
class TestAlignColumnSchemas:
    ...
    def test_handles_no_new_columns(self):
        ...
        existing = pd.DataFrame(
            {"A": [1], "B": [2], "Best Bookmaker": ["Bookmaker A"], "Result": [3]}
        )
        new = pd.DataFrame(
            {"A": [4], "B": [5], "Best Bookmaker": ["Bookmaker A"], "Result": [6]}
        )
...
class TestSaveBettingData:
    ...
    def test_prints_when_print_bets_true(self, capsys):
        ...
        with patch("src.find_bets.file_management.pd.DataFrame.to_csv"):
            save_betting_data(
                existing, new, "test.csv", "Expected Value", print_bets=True
            )
    ...
    def test_does_not_print_when_print_bets_false(self, capsys):
        ...
        with patch("src.find_bets.file_management.pd.DataFrame.to_csv"):
            save_betting_data(
                existing, new, "test.csv", "Expected Value", print_bets=False
            )
